...
void CommandLine(int FeedbackHandle, char *Command, 
				 char *Params){
	//if (!Command || strisspace(std::string(Command))){
	//	IDE_CommandFeedback(FeedbackHandle,"ORA-20001: file is null;");
	//	return;
	//}
	// IDE_CommandFeedback(FeedbackHandle,_strdup(PckDir.c_str()));
	std::string pckfile = PckDir.c_str();
    pckfile += "/";
    pckfile += Command;
	//int res = _access(pckfile.c_str(),0);
	//if (res == ENOENT || res == EACCES) {
	//	std::string t = std::string("ORA-20001: ")+Command+" file not exists!";
	//	IDE_CommandFeedback(FeedbackHandle,_strdup(t.c_str()));
	//	return;
	//}
	if (IDE_OpenFile(wtAuto, _strdup(pckfile.c_str()))) {
	    IDE_Perform(pcmd_Execute);
		std::string str = "CMDPCK compile '";
		str += pckfile;
		str += "' success.";
	    IDE_CommandFeedback(FeedbackHandle,_strdup(str.c_str()));
	} else {
		std::string str = "ORA-20001: Can not open file '";
		str += pckfile;
		str += "'.";
	    IDE_CommandFeedback(FeedbackHandle,_strdup(str.c_str()));
	}
...
void OnCreate()
{ 
	std::string CfgFile = getenv("USERPROFILE");
	CfgFile +=  "/cmdpck/config.ini";
	char TmpPckDir[100] ;
	int res = _access(CfgFile.c_str(),0);
	if (res == ENOENT || res == EACCES) {
		return;
	}
	
	std::ifstream fin(CfgFile, std::ios::in);
	while (fin.getline(TmpPckDir,100)) {
		if (TmpPckDir && !strisspace(std::string(TmpPckDir))) {
			break;
		}
	}
	fin.close();
	...
	//res = _access(TmpPckDir,0);
	//if (res == ENOENT || res == EACCES) {
	//	return;
	//}
	PckDir = std::string(TmpPckDir);
}
////---------------------------------------------------------------------------

...
void  (*funcptr_IDE_CommandFeedback)(int FeedbackHandle , char *S) = NULL;
void RegisterCallback(int Index, void *Addr)
{
    ...
    case 47 :
		...
		break;
    case 180 :
		memcpy(&funcptr_IDE_CommandFeedback, &Addr, sizeof(Addr));
		break;
	}
}
...
void  IDE_CommandFeedback(int FeedbackHandle , char *S) {
	if (!funcptr_IDE_CommandFeedback) 
	{
		ErrorMessage("IDE_CommandFeedback");
		return;
	}
	funcptr_IDE_CommandFeedback(FeedbackHandle, S);
}
